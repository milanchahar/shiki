...
To enable [TypeScript Twoslash](/packages/twoslash) (type hover on code snippets) in VitePress, we provide a VitePress plugin for easy setup. Pre-styled, with [Floating Vue](https://floating-vue.starpad.dev/) to display the type information **outside** of the code container.
...
In addition, this plugin also integrates [`twoslash-vue`](https://twoslash.netlify.app/packages/vue) for you, so that you can also highlight Vue SFC blocks with `vue twoslash`:
...
export default defineConfig({
  markdown: {
    codeTransformers: [
      // Enable Shiki Twoslash for type info
      transformerTwoslash({
